...
router.post('/2fa/confirm', authenticate, authLimiter, controller.confirm2fa)
router.post('/2fa/disable', authenticate, authLimiter, validate(validators.disable2faSchema), controller.disable2fa)

router.post('/refresh', authLimiter, validate(validators.refreshTokenSchema), controller.refresh)
...
router.post('/forgot-password', authLimiter, validate(validators.forgotPasswordSchema), controller.forgotPassword)
router.post('/reset-password', authLimiter, validate(validators.resetPasswordSchema), controller.resetPassword)
router.post('/change-password', authenticate, authLimiter, validate(validators.changePasswordSchema), controller.changePassword)

router.post('/send-verification', authenticate, authLimiter, controller.sendVerification)
router.post('/verify-email', authLimiter, validate(validators.verifyEmailSchema), controller.verifyEmail)
