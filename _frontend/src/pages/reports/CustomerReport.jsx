...
export default function CustomerReport() {
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => reportService.getCustomers().then(r => r.data),
    staleTime: 60_000
  })
  ...
  const handleExport = async (format) => {
    try {
      ...
      a.href = url
      a.download = `Customers_Report.${ext}`
  ...
  return (
    <>
      <Helmet><title>Customer Reports — MSME BMS</title></Helmet>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Customer Loyalty & Statement Reports</h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-sm text-muted-foreground">Total Customer Directory</p>
              <p className="text-2xl font-bold mt-1 text-indigo-600">
                {isLoading ? <Skeleton className="h-8 w-16 mx-auto" /> : summary.totalCustomers || 0}
              </p>
            </CardContent>
          </Card>
          <Card>
        ...
        {/* Details Table */}
        <Card>
          <CardHeader><CardTitle className="text-base font-semibold">Active Customer Registry</CardTitle></CardHeader>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-muted/50 border-b">
                    <th className="px-4 py-2 text-left font-semibold text-muted-foreground">Customer</th>
                    <th className="px-4 py-2 text-right font-semibold text-muted-foreground">Loyalty Points</th>
                    <th className="px-4 py-2 text-right font-semibold text-muted-foreground">Account Balance</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item, idx) => (
                  ...
                  {items.length === 0 && (
                    <tr>
                      <td colSpan={3} className="px-4 py-6 text-center text-muted-foreground">No customer records found</td>
                    </tr>
                  )}
                </tbody>
              </table>
